...
\textbf{Thời gian thực tập:} & 18/03/2026 -- 30/04/2026 \\
...
\hline
\textbf{Nhóm dữ liệu} & \textbf{File đầu vào/đầu ra chính} & \textbf{Vai trò} \\
\hline

Giá lịch sử &
\seqsplit{data\_Histo/historical\_price\_all.parquet} &
Dữ liệu OHLCV của cổ phiếu và chỉ số \\
\hline

VN-Index &
\seqsplit{data\_Histo/vnindex\_eda\_output.parquet} &
Dữ liệu riêng cho VN-Index sau khi lọc và kiểm tra \\
\hline

Tin tức thô &
\seqsplit{data\_News/dataset\_news.parquet} &
Dữ liệu tin tức ban đầu \\
\hline

Tin tức sạch &
\seqsplit{data\_News/clean\_news.parquet} &
Dữ liệu sau khi chuẩn hóa ngày, domain, category và loại nhiễu \\
\hline

Tin cổ phiếu &
\seqsplit{data\_News/equity\_news.parquet} &
Tin tức được lọc theo domain và category liên quan cổ phiếu \\
\hline

Nội dung sạch &
\seqsplit{data\_News/equity\_news\_clean\_content.parquet} &
Nội dung bài báo sau xử lý trùng lặp và nhiễu \\
\hline

Nội dung đã tách từ &
\seqsplit{data\_News/equity\_news\_tokenized\_vncorenlp.parquet} &
Đầu vào cho trích xuất n-gram và mô hình NLP \\
\hline

Dictionary sentiment &
\seqsplit{data\_News/candidate\_ngram\_terms\_dictionary.parquet} &
Từ điển sentiment tài chính theo term \\
\hline

Sentiment theo bài báo &
\seqsplit{data\_News/equity\_news\_content\_sentiment\_ratios.parquet} &
Dữ liệu bài báo có sentiment score, ratio và label \\
\hline

Sentiment index &
\seqsplit{data\_News/market\_sentiment\_index\_daily.parquet};
\seqsplit{data\_News/market\_sentiment\_index\_weekly.parquet} &
Chỉ số sentiment theo thời gian \\
\hline

Dữ liệu phân tích tác động &
\seqsplit{data\_News/vnindex\_weekly\_sentiment\_merged.parquet};
\seqsplit{data\_News/vnindex\_weekly\_sentiment\_abnormal\_return.parquet} &
Dữ liệu merge giữa sentiment và VN-Index \\
\hline

\end{tabularx}
\caption{Các nguồn dữ liệu chính trong pipeline}
\end{table}
...
## Lọc dữ liệu theo danh sách mã cần dùng
...
for candidate_path in candidate_paths:
    ...
    raise FileNotFoundError("Không tìm thấy file data_Histo/vnindex_eda_output.csv")
...
| `net_positive_article_ratio` | Các target tương tự |
...
| Sentiment variable | Target | Pearson | Spearman | Diễn giải |
|---|---:|---:|---:|---|
| `sentiment_index_z` | `future_ret_1w` | -0,013 | -0,007 | Gần như không có tương quan tuyến tính/đơn điệu |
| `sentiment_index_z` | `future_ret_4w` | -0,102 | -0,106 | Tương quan âm nhẹ ở horizon 4 tuần |
| `sentiment_index_z` | `future_abnormal_rolling_ret_4w` | -0,129 | -0,116 | Tín hiệu âm rõ hơn với abnormal return 4 tuần |
| `sentiment_index_z` | `future_abnormal_ar1_ret_4w` | -0,104 | -0,082 | Tương quan âm nhẹ |
...
| `future_abnormal_rolling_ret_1w` | -0,0009 | -- | 0,415 | 0,0066 | 709 | Không có ý nghĩa thống kê |
| `future_abnormal_rolling_ret_4w` | -0,0105 | -- | 0,0029 | 0,0416 | 706 | Có ý nghĩa thống kê, tác động âm |
| `future_abnormal_ar1_ret_1w` | -0,0004 | -- | 0,727 | 0,0037 | 694 | Không có ý nghĩa thống kê |
| `future_abnormal_ar1_ret_4w` | -0,0076 | -- | 0,0193 | 0,0211 | 691 | Có ý nghĩa thống kê, tác động âm |
...
## Sentiment có tác động mạnh không?
...
## Vì sao R² thấp vẫn có ý nghĩa?

Trong tài chính, R² của mô hình dự báo return thường thấp vì return rất nhiễu. Một biến có R² thấp vẫn có thể hữu ích nếu:
...
Vì vậy, R² không phải thước đo duy nhất. Để xem sentiment có tác động hay không, cần nhìn đồng thời:

| Chỉ số | Vai trò |
|---|---|
| Coefficient | Cho biết chiều và độ lớn tác động |
| t-stat/p-value | Cho biết hệ số có ý nghĩa thống kê hay không |
| R²/Adjusted R² | Cho biết mô hình giải thích được bao nhiêu phần biến động |
...
## Backtesting tín hiệu giao dịch
